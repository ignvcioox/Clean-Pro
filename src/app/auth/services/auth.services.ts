...
import { User, AuthResponse, VerifyEmailResponse } from '@auth/interfaces';
...
export class AuthService {
   ...
   */
   ...
   /**
    * Verifica el correo electrónico del usuario mediante un código.
    * @param email Correo electrónico del usuario.
    * @param code Código de verificación enviado al correo.
    * @return Observable que emite true si la verificación es exitosa.
    * @author Benjamín López
    */
   verifyEmail(email: string, code: string): Observable<boolean> {
      return this.http.post<VerifyEmailResponse>(`${baseUrl}/auth/verify-email`, {
         email,
         code,
      }).pipe(
         map(() => true),
